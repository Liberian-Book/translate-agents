...
1. **Bất khả xâm phạm:** Tuyệt đối KHÔNG xóa, thay đổi hay làm hỏng thuộc tính `id`, `class` của bất kỳ thẻ nào.
2. **Giữ nguyên inline tags:** Các thẻ `<em>`, `<strong>`, `<a href="...">`, `<span data-type="term">`... **phải được giữ nguyên vị trí và vai trò** trong câu tiếng Việt. Không được làm mất thẻ.
3. **Cặp song ngữ cân bằng:** Mỗi thẻ `eng hidden` phải có đúng 1 thẻ `vn visible` tương ứng. Không thêm, không bớt.
...
✅ Lưu ý: `<span data-type="term">` được giữ nguyên, `id` thẻ vn có hậu tố `-vn`, nội dung bên trong đã dịch theo glossary.
