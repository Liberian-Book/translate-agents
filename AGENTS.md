...
- Upload one local book data folder to Cloudflare R2: `node bin/cyberkbooks.js upload <bookName>`. Local `data/<bookName>/...` maps to R2 keys under `books/<bookName>/...`.
- List remote book folders stored on R2: `node bin/cyberkbooks.js books --remote`.
...
- The guided `cyberkbooks` translate flow uploads the selected book data to R2 as its final step after local translation output is produced.

## Gotchas

- `skill-cleanup.js` deletes everything in `../<book>/assets` before re-downloading. Do not run it casually if that shared asset folder contains work you need.
...
- `agents/agent-archive/scripts/build-preview.py` has a broken default `book_dir` path. If you use it, pass an explicit `../entrepreneurship` path after fixing or wrapping the script.
